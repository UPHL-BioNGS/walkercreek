...
sub usage {
	...
	USAGE:	flu_create_sample_sheet.pl -i <input directory> -o <output>
...
if ( $version ) 	{ die "flu_create_sample_sheet.pl version 0.1.0\n"; }
if ( $help    )     { die usage();											}

################################################################################
# Read the input directory, capturing all files that end in .fastq (optionally .gz) or some flavor of this extension
opendir( my $dh, $input ) or die "flu_create_sample_sheet::ERROR --> Cannot read the given input directory.  $!\n";
my @fastqs;
while ( readdir $dh )	{
	chomp $_;
...
# Taken from (https://github.com/CDCgov/mycosnp-nf/blob/master/bin/mycosnp_create_sample_sheet.pl)
