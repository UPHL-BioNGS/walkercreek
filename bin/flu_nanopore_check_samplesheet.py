...
# This script is based on the example at https://github.com/peterk87/nf-flu/blob/master/bin/check_sample_sheet.py
...
def adjust_reads_path(p: str) -> str:
    assert (
        p.endswith(".fastq") or p.endswith(".fastq.gz") or p.endswith(".fq") or p.endswith(".fq.gz")
...
def main(input_path: Path, platform: str, output_sample_sheet: Path):
    ...
    logging.info(f'input_path="{input_path}" output_sample_sheet="{output_sample_sheet}"')
    ...
    try:
        ...
        raise ex
    if platform == "flu_illumina":
        assert df.shape[1] == 3, f"3 columns expected in sample sheet, but {df.shape[1]} found!"
