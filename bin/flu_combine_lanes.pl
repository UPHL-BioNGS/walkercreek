...
sub usage {
	my $usage = "\nflu_combine_lanes.pl\nv0.1.0\n
	PURPOSE: Reads and parses a CSV sample sheet from flu_create_sample_sheet.pl to combine lanes with system calls to 'cat',
		 Then creates a new CSV sample sheet with the fields <name>, <r1>, <r2>

	USAGE:	flu_combine_lanes.pl -i <input sample sheet> -o <output sample sheet>
...
if ( $version ) 	{ die "flu_combine_lanes.pl version 0.1.0\n"; }
if ( $help    )     { die usage();											}
...
# taken from (https://github.com/CDCgov/mycosnp-nf/blob/master/bin/mycosnp_combine_lanes.pl)
