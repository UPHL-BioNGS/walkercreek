process IRMA_RSV {
    ...
    if [ -d "${prefix}" ] && [ -n "\$(ls -A ${prefix}/*.fasta)" ]; then
        echo "\$(basename \$(find ${prefix} -name "*.fasta" | head -n1 | rev | cut -d_ -f1 | rev))" > "${prefix}_SUBTYPE"
    else
        echo "NoIRMAsubtype " > "${prefix}_SUBTYPE"
    fi

    if [ -s "${prefix}_SUBTYPE" ]; then
        cat "${prefix}_SUBTYPE" > "${prefix}.subtype.txt"
