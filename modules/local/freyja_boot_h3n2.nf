process FREYJA_BOOT_H3N2 {
    ...
    output:
    tuple val(meta), path("${meta.id}.h3n2.boot.lineages.tsv"),   optional: true, emit: h3n2_boot_lineages
    tuple val(meta), path("${meta.id}.h3n2.boot.summarized.tsv"), optional: true, emit: h3n2_boot_summarized
    ...
    "${task.process}":
        freyja_boot_h3n2: \$(freyja --version 2>&1 | sed 's/^.*version //')
    END_VERSIONS
    }
    ...
    if ! awk '{ if (\$4 > 0) { found=1; exit } } END { exit (found?0:1) }' "${depths}"; then
        echo "Skipping freyja boot (H3N2) for ${meta.id}: depths are all zero (no coverage)."
        write_versions
        exit 0
    fi

    prefix="${meta.id}.h3n2.boot"
    ...
    if freyja boot --barcodes "${barcodes}" --output "\${prefix}" "${variants}" "${depths}" ; then
        ...
        echo "Freyja boot (H3N2) failed for ${meta.id}; no outputs will be emitted." >&2
        write_versions
        exit 0
    fi

    if [[ -f "\${prefix}_lineages.tsv" ]]; then
        mv "\${prefix}_lineages.tsv" "${meta.id}.h3n2.boot.lineages.tsv"
    fi
    if [[ -f "\${prefix}_summarized.tsv" ]]; then
        mv "\${prefix}_summarized.tsv" "${meta.id}.h3n2.boot.summarized.tsv"
    fi
