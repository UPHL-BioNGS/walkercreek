process FREYJA_BOOT_H5NX {
    ...
    output:
    tuple val(meta), path("${meta.id}.h5nx.boot.lineages.tsv"),   optional: true, emit: h5nx_boot_lineages
    tuple val(meta), path("${meta.id}.h5nx.boot.summarized.tsv"), optional: true, emit: h5nx_boot_summarized
    ...
    "${task.process}":
        freyja_boot_h5nx: \$(freyja --version 2>&1 | sed 's/^.*version //')
    END_VERSIONS
    }
    ...
    if ! awk '{ if (\$4 > 0) { found=1; exit } } END { exit (found?0:1) }' "${depths}"; then
        echo "Skipping freyja boot (H5NX) for ${meta.id}: depths are all zero (no coverage)."
        write_versions
        exit 0
    fi

    prefix="${meta.id}.h5nx.boot"
    ...
    if freyja boot --barcodes "${barcodes}" --output "\${prefix}" "${variants}" "${depths}" ; then
        ...
        echo "Freyja boot (H5NX) failed for ${meta.id}; no outputs will be emitted." >&2
        write_versions
        exit 0
    fi

    if [[ -f "\${prefix}_lineages.tsv" ]]; then
        mv "\${prefix}_lineages.tsv" "${meta.id}.h5nx.boot.lineages.tsv"
    fi
    if [[ -f "\${prefix}_summarized.tsv" ]]; then
        mv "\${prefix}_summarized.tsv" "${meta.id}.h5nx.boot.summarized.tsv"
    fi
