process FREYJA_DEMIX_B_VIC {
    ...
    output:
    tuple val(meta), path("${meta.id}.b_vic.tsv"), optional: true, emit: demix_b_vic
    ...
    "${task.process}":
        freyja_demix_b_vic: \$(freyja --version 2>&1 | sed 's/^.*version //')
    END_VERSIONS
    } 

    # Skip if depths are all zero (no coverage)
    if ! awk '{ if (\$4 > 0) { found=1; exit } } END { exit (found?0:1) }' "${depths}"; then
        echo "Skipping freyja demix (B_VIC) for ${meta.id}: depths are all zero (no coverage)."
        write_versions
        exit 0
    fi

    if ! freyja demix --output "${meta.id}.b_vic.tsv" --barcodes "${barcodes}" "${variants}" "${depths}"; then
        echo "Freyja demix (B_VIC) failed for ${meta.id}; skipping output." >&2
        write_versions
        exit 0
    fi
