process ALIGN_TO_REFS_NANOPORE {
    tag "$meta.id"
    label 'process_high'
    ...
    """
    [ ! -f ${prefix}.clean.fastq.gz ] && ln -s ${reads} ${prefix}.clean.fastq.gz

    # Align to H1N1 with Nanopore preset
    minimap2 -ax map-ont -t ${task.cpus} $h1n1_freyja_ref ${prefix}.clean.fastq.gz | samtools sort -o ${prefix}_H1N1.sort.bam
    samtools index ${prefix}_H1N1.sort.bam
    samtools coverage ${prefix}_H1N1.sort.bam > ${prefix}_H1N1.covstats

    # Align to H3N2
    minimap2 -ax map-ont -t ${task.cpus} $h3n2_freyja_ref ${prefix}.clean.fastq.gz | samtools sort -o ${prefix}_H3N2.sort.bam
    samtools index ${prefix}_H3N2.sort.bam
    samtools coverage ${prefix}_H3N2.sort.bam > ${prefix}_H3N2.covstats

    # Align to H5Nx
    minimap2 -ax map-ont -t ${task.cpus} $h5nx_freyja_ref ${prefix}.clean.fastq.gz | samtools sort -o ${prefix}_H5Nx.sort.bam
    samtools index ${prefix}_H5Nx.sort.bam
    samtools coverage ${prefix}_H5Nx.sort.bam > ${prefix}_H5Nx.covstats

    # Align to B_VIC
    minimap2 -ax map-ont -t ${task.cpus} $b_vic_freyja_ref ${prefix}.clean.fastq.gz | samtools sort -o ${prefix}_b_vic.sort.bam
