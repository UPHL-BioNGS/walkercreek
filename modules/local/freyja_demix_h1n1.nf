process FREYJA_DEMIX_H1N1 {
    ...
    output:
    tuple val(meta), path("${meta.id}.h1n1.tsv"), optional: true, emit: demix_h1n1
    ...
    "${task.process}":
        freyja_demix_h1n1: \$(freyja --version 2>&1 | sed 's/^.*version //')
    END_VERSIONS
    }

    # Skip if depths are all zero (no coverage)
    if ! awk '{ if (\$4 > 0) { found=1; exit } } END { exit (found?0:1) }' "${depths}"; then
        echo "Skipping freyja demix (H1N1) for ${meta.id}: depths are all zero (no coverage)."
        write_versions
        exit 0
    fi

    if ! freyja demix --output "${meta.id}.h1n1.tsv" --barcodes "${barcodes}" "${variants}" "${depths}"; then
        echo "Freyja demix (H1N1) failed for ${meta.id}; skipping output." >&2
        write_versions
        exit 0
    fi
